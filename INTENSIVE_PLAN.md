...
(Не пугайтесь, что выглядит страшно, просто я по пунктам разбивал буквально **всё**, поэтому оно так сгенерировалось)
...
- [План интенсива по созданию AI-стримерши](#план-интенсива-по-созданию-ai-стримерши)
  ...
  - [Блок 2. Запуск ИИ-персонажа (Хост)](#блок-2-запуск-ии-персонажа-хост)
    ...
    - [2.3 Кастомизация](#23-кастомизация)
      - [2.3.1 Кастомизация VTube Studio-модели, скачивание своей модели](#231-кастомизация-vtube-studio-модели-скачивание-своей-модели)
      - [2.3.2 Кастомизация OBS сцены](#232-кастомизация-obs-сцены)
      - [2.3.3 Кастомизация кода в Python-скрипте](#233-кастомизация-кода-в-python-скрипте)
    - [2.4 Запуск](#24-запуск)
  - [Блок 3. Проработка личности персонажа](#блок-3-проработка-личности-персонажа)
    - [3.1 Базовая часть](#31-базовая-часть)
      - [3.1.1 Ник персонажа](#311-ник-персонажа)
      - [3.1.2 Описание персонажа](#312-описание-персонажа)
      - [3.1.3 Примеры ответов персонажа](#313-примеры-ответов-персонажа)
  ...
  - [Блок 5. Оформление](#блок-5-оформление)
    - [5.1 Оформление канала Twitch](#51-оформление-канала-twitch)
      - [5.1.1 Аватарка канала](#511-аватарка-канала)
      - [5.1.2 Шапка канала](#512-шапка-канала)
      - [5.1.3 Описание канала](#513-описание-канала)
  - [Блок S. Наблюдение](#блок-s-наблюдение)
    - [S1. Играем на сервере](#s1-играем-на-сервере)
    - [S2. Документируем процесс](#s2-документируем-процесс)
  - [F. Финальный блок](#f-финальный-блок)
    - [F1. Финально синхронизируемся](#f1-финально-синхронизируемся)
    - [F2. Залетаем на публичный сервер](#f2-залетаем-на-публичный-сервер)
...
flowchart TD
    ...
    A2 --> A
    A4[Для роли Хост: выбрать трек разработки и заполнить форму ОСНОВНОЙ РЕГИСТРАЦИИ ХОСТА] --> A5[Скачать тяжёлые программы и файлы заранее] --> A
...
4. Установите **Minecraft** с **модами** согласно инструкции и минимально освойтесь ([инструкция](docs/instructions/Minecraft.md))
...
После выбора трека ОБЯЗАТЕЛЬНО заполните [формочку](https://forms.gle/VxHGzWpPp7PaNec78) **ОСНОВНОЙ** регистрации для ХОСТА
...
- Строго следовать [инструкциям](https://3ndetz.github.io/NetTyan/getting-started/) по установке
- Минимально [настроить](https://3ndetz.github.io/NetTyan/1_CONFIG/) и запустить, проверить, чтоб работало
  - [Кастомизировать](https://3ndetz.github.io/NetTyan/CUSTOMIZATION/) будем ужее далее, сначала просто ЗАПУСТИТЬ, чтобы коннектилось с игрой и т.д.
  - LLM уже можно запустить, бесплатный Mistral LLM API работающий без *сервисов смены интернет-региона*, (API ключ получают в пункте задачи 4.3 этого плана, если есть сокомандник, пусть регает, если нет - регаешь сам)

(P. S. репозиторий не на гите, так как это немного урезанная альфа версия нашей моно-репки ИИ-стримерши, мы не готовы пока его публиковать...)

### 2.3 Кастомизация

| ID | ID pre | Роли | Важность |
|----|----|------|-------------|
| 2.3 | Все блоки 2, 3, 4, 5 (что есть) | Хост | Важно |

#### 2.3.1 Кастомизация VTube Studio-модели, скачивание своей модели

  | ID | ID pre | Роли | Важность |
  |----|----|------|-------------|
  | 2.3.1 | 3.3 | Хост | Опционально |

  [Инструкция по смене модели](docs/instructions/VtubeModel.md)

#### 2.3.2 Кастомизация OBS сцены

  | ID | ID pre | Роли | Важность |
  |----|----|------|-------------|
  | 2.3.2 | 3.2, 3.3 | Хост | Опционально |

  Можно докинуть элементов-картинок из блока 3.3, подходящего текста под персонажа (3.2), интегрировать донаты (4.4)

  [Инструкция OBS](docs/instructions/OBS.md), [Гайд](https://prostrimer.ru/obs-studio/polnyij-obzor-po-sczenam-i-istochnikam-v-obs-studio) по основам сцен OBS

#### 2.3.3 Кастомизация кода в Python-скрипте

  | ID | ID pre | Роли | Важность |
  |----|----|------|-------------|
  | 2.3.3 | 3.1.3, 3.2, 4.2 | Хост | Важно |

  (см. [тут](https://3ndetz.github.io/NetTyan/CUSTOMIZATION/))
...
Игровой ник персонажа, соответствующий игровым требованиям.

К нему есть требования:

- не короче 4 и не длиннее 10 символов
- не простой типа "apple"
- пожалуйста, не что-то вроде "fdrjwetjre", или "124325"

#### 3.1.2 Описание персонажа

Характер, манера общения, поведение, привычки, интересы, цели и мотивация. В идеале: референсы персонажа, какие-то черты, например, "шутит как ...".

#### 3.1.3 Примеры ответов персонажа

Составьте эталонные примеры ответов персонажа в разных сценариях:
...
API ключи (Mistral бесплатный и можно зарегать по РУ номеру), GROQ и GEMINI тоже крутые лимиты, но если есть *сервис смены региона*
...
- [ImageGen-сервисы](https://huggingface.co/spaces) в HuggingFace Spaces
- [Perchance AI](https://perchance.org/ai-art-generator-advanced) полностью бесплатный и вообще без лимитов (во всех смыслах, кстати)
...
| ID | ID pre | Роли | Важность |
|----|----|------|-------------|
| S | - | Наблюдатель (и периодически все) | Важно |

### S1. Играем на сервере

Заходим на сервер со своего ника.

Комментируем наработки ребят, да и просто общаемся, весело проводим время

### S2. Документируем процесс

Вносим что-нибудь интересное в заметки или в гугл док

Записываем какие-нибудь видосы на память (особенно на публичных серверах!)
...
Все заходим на наш сервер, синхронизируемся, пока просто встречаемся, смотрим, что агенты работают

### F2. Залетаем на публичный сервер

Ура, мы всё проверили, всё работает, залетаем к живым людям прямо группой агентов, смотрим на их реакцию и т.д.!
