...
Возле шестерёнки ждём на значок Steam → Жмём на **Фильтр** → Указываем "ранее загруженный или отслеживаемый контент" → выбираем модель и жмём ок, она будет загружаться.

---

## 🎤 Настройка lipsync (синхронизация губ)

### Тестируем lipsink через ВАШ микрофон
...
Если этого не происходит, надо настраивать параметры самой модели на Voice, либо проверять микрофон.

Когда всё работает, можно подключать виртуальный кабель (TTS).

### Через виртуальный кабель (для TTS)
